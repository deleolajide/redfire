...
public class Red5Plugin implements Plugin, ChatRoomListener, PacketListener, PacketFilter, TranscriptWindowInterceptor
{
    ...
    public Red5Plugin()
    {
		...
		mdnsServiceListener = new ServiceListener()
		{
			...
			{
				jmdnsClient.requestServiceInfo(serviceEvent.getType(), serviceEvent.getName(), 1);

				String serviceUrl = serviceEvent.getInfo().getURL();
				Log.warning("serviceAdded - " + serviceUrl);
			}
			...
			{
				String serviceUrl = serviceEvent.getInfo().getURL();
				Log.warning("serviceResolved - " + serviceUrl);

				url = serviceUrl;
				String server = serviceUrl.substring(serviceUrl.indexOf("//") + 2);
				red5port = server.substring(server.indexOf(":") + 1);
				red5server = server.substring(0, server.indexOf(":"));

				Log.warning("mDNS discovery server " + red5server + " " + red5port);

				jmdnsClient.removeServiceListener("_http._tcp.local.", mdnsServiceListener);
				jmdnsClient.close();
			}
		};

		try {
			jmdnsClient = new JmDNS();
			jmdnsClient.addServiceListener("_http._tcp.local.", mdnsServiceListener);

		} catch (Exception e) {

			Log.error("jmdnsClient creation " + e);
		}
    }
