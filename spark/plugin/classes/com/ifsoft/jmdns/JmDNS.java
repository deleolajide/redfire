...
public class JmDNS
{
    ...
    private class Logger {
		...
		public void debug(String message) { Log.warning(message); }

		public void info(String message) { Log.warning(message); }

		public void warn(String message) { Log.warning(message); }

		public void error(String message) { Log.error(message); }

		public void warn(String message, Throwable e) { Log.warning(message + e); }

		public void error(String message, Throwable e) { Log.error(message + e); }
	}
        ...
        throws IOException
    {
        ...
        boolean serviceConflictDetected = false;
            ...
            logger.debug("JMDNS/handleResponse received " +
            msg.answers.size()+ " messages");
        for (DNSRecord rec : msg.answers)
        {

              logger.debug("PRINT: "+ rec);
          //cache.add(rec);
        }
        ...
        for (DNSRecord rec : msg.answers)
        {
            ...
            boolean expired = rec.isExpired(now);
                ...
                logger.debug("JMDNS received : " + rec + " expired: "+expired);
            ...
            if (c != null)
            {
                    ...
                    logger.debug("JMDNS has found "+rec+" in cache");
                if (expired)
                {
                    isInformative = true;
                    cache.remove(c);
            ...
            else
            {
                if (!expired)
                {
                    isInformative = true;
                        ...
                        logger.debug("Adding "+rec+" to the cache");
                    cache.add(rec);
                }
            }
            switch (rec.type)
                ...
                while (state != DNSState.CANCELED)
                {
                    ...
                    try
                    {
                        ...
                        DNSIncoming msg = new DNSIncoming(packet);
                            ...
                            logger.debug("SocketListener.run() JmDNS in:" +
                            msg.print(true));
