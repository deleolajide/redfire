...
{
	public ManageVoiceMessage(VoiceBridgeComponent voicebridgeComponent)
	{
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			String errorMessage = this.getVoiceBridgeComponent().manageVoiceMessage(newCommand, profileID, featureID, action, value1);
		...
		{
			Log.error("[VoiceBridgeOpenlink] Manage Voice Message execute error " + e);
