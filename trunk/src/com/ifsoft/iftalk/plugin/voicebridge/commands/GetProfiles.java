...
{
	public GetProfiles(VoiceBridgeComponent voicebridgeComponent) {
	...
	public Element execute(SessionData data, Element newCommand,Element oldCommand)
	{
		...
		try {
			...
			List<VoiceBridgeUser> voicebridgeUsers = this.getVoiceBridgeComponent().getOpenlinkProfiles(userJID);
			Iterator it = voicebridgeUsers.iterator();

			while( it.hasNext() )
			{
				VoiceBridgeUser voicebridgeUser = (VoiceBridgeUser)it.next();

				String profileID = voicebridgeUser.getUserNo();

				Element genericProfile = profiles.addElement("profile");
				genericProfile.addAttribute("id", profileID);
				genericProfile.addAttribute("label", voicebridgeUser.getUserName());
				genericProfile.addAttribute("default", voicebridgeUser.getDefault());
				genericProfile.addAttribute("online", "0000000000".equals(voicebridgeUser.getDeviceNo()) ? "false" : "true");
				...
				genericActions.addElement("action").addAttribute("id","IntercomTransfer").addAttribute("label",		"Perform a transfer internally using VoiceBridge Intercom on active call");
				...
				genericActions.addElement("action").addAttribute("id","SendDigvoicebridge").addAttribute("label",		"Send dial digits on selected line");
				...
				if (this.getVoiceBridgeComponent().voicebridgeLinkService.isCallbackAvailable())
				{
