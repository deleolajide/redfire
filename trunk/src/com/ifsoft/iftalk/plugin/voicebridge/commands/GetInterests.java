...
public class GetInterests extends OpenlinkCommand {
	...
	public GetInterests(VoiceBridgeComponent voicebridgeComponent) {
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			if (voicebridgeUser != null)
			{
				...
				Iterator it = voicebridgeUser.getInterests().values().iterator();

				while( it.hasNext() )
				{
					VoiceBridgeInterest voicebridgeInterest = (VoiceBridgeInterest)it.next();
					String interestID = voicebridgeInterest.getInterestId() + voicebridgeUser.getUserNo();
					...
					if (voicebridgeInterest.getUserInterests().containsKey(voicebridgeUser.getUserNo()))
					{
