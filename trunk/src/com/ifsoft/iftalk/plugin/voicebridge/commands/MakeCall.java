...
public class MakeCall extends OpenlinkCommand {
	...
	// private VoiceBridgeComponent voicebridgeComponent;

	public MakeCall(VoiceBridgeComponent voicebridgeComponent) {
	...
	{
		try {
			Log.info("[voicebridge] Make Call execution");
			...
			if (in.element("interest") != null)
			{
				...
				if (!"".equals(userInterest))
				{
					...
					if (voicebridgeUserInterest != null)
					{
						...
						if (!userJID.getNode().equals(voicebridgeUserInterest.getUser().getUserId()))
						{
				...
				for ( Iterator i = in.element("features").elementIterator( "feature" ); i.hasNext(); )
				{
					...
					if (pos > - 1)
					{
						...
						if ("sd".equals(featureType))	// speed dial
						{
							...
							if (!this.getVoiceBridgeComponent().voicebridgeLdapService.speedDialTable.containsKey(speedDial))
						...
						if ("hs".equals(featureType))	// handset
						{
							...
							if (!this.getVoiceBridgeComponent().validateTrueFalse(featureValue))
						...
						if ("priv".equals(featureType))	// privacy
						{
							...
							if (!this.getVoiceBridgeComponent().validateTrueFalse(privacy))
						...
						if ("hold".equals(featureType))	// auto-hold
						{
							...
							if (!this.getVoiceBridgeComponent().validateTrueFalse(autoHold))
			...
			if (errorMessage == null)
			{
				...
				{
					VoiceBridgeSpeedDial voicebridgeSpeedDial = this.getVoiceBridgeComponent().voicebridgeLdapService.speedDialTable.get(speedDial);
					dialDigits = voicebridgeSpeedDial.getDialableNumber();
				}

				if (userInterest == null || "".equals(userInterest))		// no interest use defaults
				{
					errorMessage = this.getVoiceBridgeComponent().makeCallDefault(newCommand, userJID, handset, privacy, autoHold, dialDigits);

				} else {

					errorMessage = this.getVoiceBridgeComponent().makeCall(newCommand, userInterest, handset, privacy, autoHold, dialDigits);
				}
			}
		...
		Log.info("[voicebridge] Make Call execution"+newCommand);
		return newCommand;
	}
