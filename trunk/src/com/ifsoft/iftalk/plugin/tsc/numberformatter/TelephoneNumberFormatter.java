...
public class TelephoneNumberFormatter
{
    ...
    private final static String nameSpaceBase = "itsmyriad.";
...
//	private Pattern[] CanonicalCountryCodePatterns = {};
	...
	public void init() throws TelephoneNumberFormatException
	{
		...
		{
		    Log.error(getLocaleDebugInfo() + "Unexpected Area Code '" + AreaCode + "' supplied with Long Distance Rule set to '" + LongDistanceRule + "'");
			initialised = false;
            throw new TelephoneNumberFormatException("Unexpected Area Code '" + AreaCode + "' supplied with Long Distance Rule set to '" + LongDistanceRule + "'");
		}

		if (!LongDistanceRule.equals( "") && AreaCode.equals(""))
		{
		    Log.error(getLocaleDebugInfo() + "Area Code must be supplied with Long Distance Rule set to '" + LongDistanceRule + "'");
			initialised = false;
            throw new TelephoneNumberFormatException("Area Code must be supplied with Long Distance Rule set to '" + LongDistanceRule + "'");
		}
	...
	/**
	 * @param dialableNumb A String representing a Telephone Number in dialable format for this Locale.
	 * @return A String representing the number in Canonical Telephone Number format.
	 * @throws TelephoneNumberFormatException
	 */
	...
	public String formatCanonicalNumber(String dialableNumb) throws TelephoneNumberFormatException
	{
		...
		try
		{
			...
			if ((i=Number.indexOf('+')) > -1)
			{
				...
				Number = this.LongDistanceAccess + applyTelephonyLocationRules(removeNonDialableChars(Number.substring(++i)), InternationalRule);
...
//			else
	...
	/**
	 * @param canonicalNumb A String representing a Telephone number in Canonical Number format.
	 * @return A String representing a Dialable Number in a dialable format for this Locale.
	 * @throws TelephoneNumberFormatException
	 */
	public String formatDialableNumber(String canonicalNumb) throws TelephoneNumberFormatException
	{
		...
		try
		{
			...
			// Check if number length is equal or less than extension number length
			if (patExtensionNumber != null)
			{
				matcher = patExtensionNumber.matcher(Number);
				if (matcher.matches())
			...
			if (ret == null)
			{
				...
				{
					// No extra zero in brackets

					// Remove all non diallable characters
					Number = removeNonDialableChars(Number);
...
// can only be in Country so we can get the number ok
						dialString = dialString + num.substring(this.getCountryCode().length());
// Skip over the G
						i += 1;
					}
					else
...
//
	...
	private void setCountryCodeList(List countryCodeList) throws TelephoneNumberFormatException
	{
		...
		// Pattern prefix and suffix to match a canonical international number with an extra "n" between country code and Adrea code
...
//			Create the Canonical pattern for this country
	...
	public void setLocale(Locale locale) throws TelephoneNumberFormatException {
		try
		{
			...
			Log.info(getLocaleDebugInfo() + "Locale set to '" + myLocale.toString() + "'");
			init();
		}
		catch(NullPointerException npe)
		{
