...
public abstract class AbstractCall
{
	...
	public String getCalledNumber(String interestType)
	{
		if ("N".equals(getPrivacy()) || ("Y".equals(getPrivacy()) && "Active".equals(getParticipation())))
		{
			...
			{
				String ddi = getDDI();
				try { ddi = String.valueOf(Integer.parseInt(getDDI()));} catch (Exception e) { }
				return ddi;

			} else {

				if ("D".equals(interestType) || "C".equals(interestType))
				{
					return(getProceedingDigits());

				} else {

					String line = getLine();
					try { line = String.valueOf(Integer.parseInt(getLine()));} catch (Exception e) { }
					return line;
				}
			}
		} else return "";
	...
	public String getCalledName(String interestType)
	{
		if ("N".equals(getPrivacy()) || ("Y".equals(getPrivacy()) && "Active".equals(getParticipation())))
		{
			...
			if ("Incoming".equals(getDirection()))
			{
				...
				if ("D".equals(interestType) || "C".equals(interestType))
				{
					theName = (getProceedingDigitsLabel());
