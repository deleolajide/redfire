package com.salesbuilder.util
{
	...
	public class FileUtility
	{
		...
		public function databaseToXML():void
		{
			...
			accountDAO.getAccounts(getAccountsResultHandler);
		}
		
		private function getAccountsResultHandler(data:Object):void
		{			
			accounts = data as ArrayCollection;
			var contactDAO:ContactDAO = new ContactDAO();
			contactDAO.getContacts(getContactsResultHandler);
		}

		private function getContactsResultHandler(data:Object):void
		{			
			contacts = data as ArrayCollection;
			var opportunityDAO:OpportunityDAO = new OpportunityDAO();
			opportunityDAO.getOpportunities(getOpportunitiesResultHandler);
		}

		private function getOpportunitiesResultHandler(data:Object):void
		{			
			opportunities = data as ArrayCollection;
			...
			for (var i:int=0; i<contacts.length; i++)
			{
				...
				xml =
					...
					"\t\t\t<managerId>" + contact.managerId + "</managerId>\n" +
				...
				if (accountContacts[contact.accountId])
					accountContacts[contact.accountId] += xml;
				else
					accountContacts[contact.accountId] = xml;
			}
			...
			for (var j:int=0; j<opportunities.length; j++)
			{
				...
				if (accountOpportunities[opportunity.accountId])
					accountOpportunities[opportunity.accountId] += xml;
				else
					accountOpportunities[opportunity.accountId] = xml;
			}
